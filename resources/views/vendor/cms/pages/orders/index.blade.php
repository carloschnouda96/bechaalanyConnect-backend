    ...
    <div class="card mx-lg-5 mx-2 py-4">
        <div class="actions">
            ...
            @if ($page['order_display'])
                <a href="{{ url(config('hellotree.cms_route_prefix') . '/' . $page['route'] . '/order') }}"
                    class="btn btn-secondary btn-sm">Order</a>
            @endif
            @if ($page['delete'] || !request()->get('admin')['admin_role_id'])
                @if (request()->get('admin')['cms_pages'][$page['route']]['permissions']['delete'])
                    <form method="post" action="{{ url(config('hellotree.cms_route_prefix') . '/' . $page['route'] . '/') }}"
                        class="d-block d-md-inline-block bulk-delete {{ $page['delete'] ? '' : 'opacity-half' }}"
                            ...
                            @if (request('custom_validation'))
                                @foreach (request('custom_validation') as $i => $validation)
                                    @if (isset($validation['value'][1]) && $validation['value'][1])
                                        <input type="hidden" name="custom_validation[{{ $i }}][constraint]"
                                            value="{{ $validation['constraint'] }}">
                                        <input type="hidden" name="custom_validation[{{ $i }}][value][0]"
                                            value="{{ $validation['value'][0] }}">
                                        @foreach ($validation['value'][1] as $value)
                                            <input type="hidden" name="custom_validation[{{ $i }}][value][1][]"
                                                value="{{ $value }}">
                                        @endforeach
                                    @endif
                                @endforeach
        ...
        <div
            ...
            <table
                class="{{ $page['server_side_pagination'] ? 'table' : 'datatable' }} {{ $page['with_export'] ? '' : 'no-export' }}">
                <thead>
                    <tr>
                        <th>
                        ...
                        @foreach ($page_fields as $field)
                            ...
                            @else
                                @php
                                    ...
                                    if (request('per_page')) {
                                        $appends_to_sort_query .= 'per_page=' . request('per_page') . '&';
                                    }
                                    if (request('custom_search')) {
                                        $appends_to_sort_query .= 'custom_search=' . request('custom_search') . '&';
                                    }
                                    ...
                                    if (request('sort_by') == $field['name']) {
                                        $appends_to_sort_query .=
                                            'sort_by_direction=' .
                                            (request('sort_by_direction') == 'asc' ? 'desc' : 'asc') .
                                            '&';
                                    } else {
                                ...
                                <th>
                                    <a
                                        {!! $page['server_side_pagination'] && $field['form_field'] != 'select multiple'
                                            ? 'href="' . url(config('hellotree.cms_route_prefix') . '/' . $page['route'] . $appends_to_sort_query) . '"'
                                            : '' !!}>{{ str_replace(['_id', '_'], ['', ' '], $field['name']) }}</a>
                                    @if ($page['server_side_pagination'])
                                        <div
                        ...
                        <tr>
                            ...
                            @foreach ($page_fields as $field)
                                ...
                                @elseif ($field['form_field'] == 'select')
                                    <td>
                                        {{-- @if ($row['statuses_id'] == 1)
                                            <div class="d-flex align-items-center">
                                                <span>
                                                    <i class="fa fa-check text-success" style="font-size: 1.5em;"></i>
                                                </span>
                                            </div>
                                        @elseif ($row['statuses_id'] == 2)
                                            <div class="d-flex align-items-center">
                                                <span>
                                                    <i class="fa fa-times text-danger" style="font-size: 1.5em;"></i>
                                                </span>
                                            </div>
                                        @elseif($row['statuses_id'] == 3)
                                            <div class="d-flex align-items-center">
                                                <span>
                                        ...
                                        @if ($row[str_replace('_id', '', $field['name'])])
                                            @if ($field['name'] == 'statuses_id' && $row['statuses_id'] == 1)
                                                <div class="d-flex align-items-center">
                                                    <span>
                                                        <i class="fa fa-check text-success" style="font-size: 1.5em;"></i>
                                                    </span>
                                                </div>
                                            @elseif ($field['name'] == 'statuses_id' && $row['statuses_id'] == 2)
                                                <div class="d-flex align-items-center">
                                                    <span>
                                                        <i class="fa fa-times text-danger" style="font-size: 1.5em;"></i>
                                                    </span>
                                                </div>
                                            @elseif ($field['name'] == 'statuses_id' && $row['statuses_id'] == 3)
                                                <div class="d-flex align-items-center">
                                                    <span>
                                                        <i class="fa fa-exclamation-circle text-warning"
                                                            style="font-size: 1.5em;"></i>
                                            ...
                                            @else
                                                <a
                                                    href="{{ url(config('hellotree.cms_route_prefix') . '/' . str_replace('_', '-', $field['form_field_additionals_1']) . '/' . $row[$field['name']]) }}">
                                ...
                                @elseif ($field['form_field'] == 'select multiple')
                                    <td>
                                        @foreach ($row[str_replace('_id', '', $field['name'])] as $i => $pivot)
                                            {{ $i ? ', ' : '' }}
                                            <a
                                                href="{{ url(config('hellotree.cms_route_prefix') . '/' . str_replace('_', '-', $field['form_field_additionals_1']) . '/' . $pivot->id) }}">
                                ...
                                @elseif ($field['form_field'] == 'map coordinates')
                                    <td>
                                        @if ($row[$field['name']])
                                            <a target="_blank"
                                                href="https://www.google.com/maps/search/?api=1&query={{ $row[$field['name']] }}"><i
                                    ...
                                    <td>
                                        <div class="d-flex align-items-center">
                                            <div class="mr-1 rounded"
                                                style="width: 22px; height: 22px; background-color: {{ $row[$field['name']] }}">
                                            </div>
                            ...
                            <td class="actions-wrapper text-right">
                                @if ($page['show'] || !request()->get('admin')['admin_role_id'])
                                    @if (request()->get('admin')['cms_pages'][$page['route']]['permissions']['read'])
                                        <a href="{{ url(config('hellotree.cms_route_prefix') . '/' . $page['route'] . '/' . $row['id']) }}"
                                            class="btn btn-secondary btn-sm {{ $page['show'] ? '' : 'opacity-half' }} mb-2">View</a>
                                    @endif
                                @endif
                                @if ($page['edit'] || !request()->get('admin')['admin_role_id'])
                                    @if (request()->get('admin')['cms_pages'][$page['route']]['permissions']['edit'])
                                        <a href="{{ url(config('hellotree.cms_route_prefix') . '/' . $page['route'] . '/' . $row['id'] . '/edit' . $appends_to_query) }}"
                                            class="btn btn-primary btn-sm {{ $page['edit'] ? '' : 'opacity-half' }} mb-2">Edit</a>
                                    @endif
                                @endif
                                @if ($page['delete'] || !request()->get('admin')['admin_role_id'])
                                    @if (request()->get('admin')['cms_pages'][$page['route']]['permissions']['delete'])
                                        <form class="row-delete d-inline-block {{ $page['delete'] ? '' : 'opacity-half' }}"
                                            method="post"
                                            action="{{ url(config('hellotree.cms_route_prefix') . '/' . $page['route'] . '/' . $row['id'] . $appends_to_query) }}"
                                            onsubmit="return confirm('Are you sure?')">
                                            @csrf
                                            <input type="hidden" name="_method" value="DELETE">
                    ...
                    <div class="card my-0 px-4 py-3">
                        ...
                        <form>
                            ...
                            @foreach ($filters as $i => $filter)
                                ...
                                <input type="hidden" name="custom_validation[{{ $i }}][table]"
                                    value="{{ $filter['form_field_additionals_1'] }}">
                                <input type="hidden" name="custom_validation[{{ $i }}][value][0]"
                                    value="{{ $filter['name'] }}">
                                @include('cms::/components/form-fields/select-multiple', [
                                    ...
                                    'value' =>
                                        isset(request('custom_validation')[$i]['value'][1]) &&
                                        request('custom_validation')[$i]['value'][1]
                                            ? request('custom_validation')[$i]['value'][1]
                                            : '',
                                    'required' => false,
                                    'description' => '',
                            ...
                            <div class="text-right">
                                <a href="{{ url(config('hellotree.cms_route_prefix') . '/' . $page['route']) }}"
