...
@section('dashboard-content')
    ...
    <div class="card p-4 mx-2 mx-sm-5">
        ...
        {{-- Copy of cms::pages/cms-page/show, with one change: receipt_image is rendered
             by _receipt.blade.php instead of the vendor's image component, because that
             component calls Storage::url() on the default (public) disk and receipts
             live on the private one. Everything else is the vendor template verbatim,
             so re-copy this file if the package's show view changes. --}}
        @foreach ($page_fields as $field)
            @if ($field['form_field'] == 'image' && $field['name'] == 'receipt_image')
                @include('cms::pages/credits-transfer/_receipt', ['row' => $row])
            @else
