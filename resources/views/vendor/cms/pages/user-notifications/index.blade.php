{{--
	Published copy of cms::pages/cms-page/index, for the Notifications list only.

	One change from the package: the three branches that interpolate a raw attribute
	(textarea, rich-textbox, and the catch-all) go through FieldDisplay::text() first.

	`user_notifications.data` is a json column that App\UserNotification casts to
	`array`, and it is the only one of the three such columns still shown on a list — the
	payload carries the notification's message, so it cannot simply be hidden. Blade's
	{{ }} is htmlspecialchars(), which is typed `string`, so opening this page 500'd with
	"htmlspecialchars(): Argument #1 ($string) must be of type string, array given".

	Nothing else differs from the vendor template. Note the @php blocks below must stay
	in block form: an inline @php() pairs with the next @endphp further down the file and
	emits everything between as raw PHP.
--}}
@php
	$filters = [];
						...
						<tr>
							...
							@foreach($page_fields as $field)
								...
								@elseif ($field['form_field'] == 'textarea')
									<td>
										<div class="max-lines">{{ \App\Services\Cms\FieldDisplay::text($row[$field['name']]) }}</div>
									</td>
								@elseif ($field['form_field'] == 'rich-textbox')
									<td>
										<div class="max-lines">{{ strip_tags(\App\Services\Cms\FieldDisplay::text($row[$field['name']])) }}</div>
									</td>
								...
								@else
									<td>{{ \App\Services\Cms\FieldDisplay::text($row[$field['name']]) }}</td>
								@endif
							@endforeach
							@if(isset($rows[0]) && isset($rows[0]['cms_draft_flag']))
								<td>
